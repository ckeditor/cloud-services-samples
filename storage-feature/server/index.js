...
app.delete( '/documents/:documentId', async ( req, res ) => {
	const { organizationId, environmentId } = req.query;
	const { documentId } = req.params;

	const { status } = await sendRequest( 'DELETE', `https://${ organizationId }.cke-cs.com/api/v4/${ environmentId }/documents/${ documentId }` );
...
app.get( '/documents/:documentId', async ( req, res ) => {
	...
	const { status, data } = await sendRequest( 'GET', `https://${ organizationId }.cke-cs.com/api/v4/${ environmentId }/documents/${ documentId }` );
...
app.get( '/documents', async ( req, res ) => {
	...
	const { status, data } = await sendRequest( 'GET', `https://${ organizationId }.cke-cs.com/api/v4/${ environmentId }/documents` );
